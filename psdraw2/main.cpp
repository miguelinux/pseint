...
const float color_shape[3]={1,1,1};
...
struct Entity {
	...
	int flecha_in; // si las flechas de entrada son mas largas que lo normal (ej, entrada en un repetitivo), se pone aca la diferencia
	...
	Entity(ETYPE _type, string _label) :type(_type),label(_label) {
		...
		} else if (type==ET_PARA||type==ET_MIENTRAS||type==ET_REPETIR) {
			n_child=1;
	...
	void EditSpecialLabel(unsigned char key) {
		if (key==GLUT_KEY_LEFT) {
			...
			if (edit_pos<label.size()) edit_pos++;
		}
	...
	void SetLabel(string _label, bool recalc=false) {
		label=_label; GetTextSize(w,h); t_w=w; t_h=h;
		if (!w) w=margin*6;
	...
	bool CheckLinkNext(int x, int y) {
		if ((type==ET_PROCESO && !next) || mouse==next || nolink) return false;
	...
	void Draw() {
		...
		glBegin(GL_LINES); 
			...
			if (!nolink) {
				...
				DrawFlechaDownHead(d_dx+x,d_dy+y); // no en inicio
			}
		glEnd();
		// relleno de la forma
		...
		glPushMatrix();
		glTranslated(d_fx-t_w/2,d_fy-d_h/2-margin,0);
...
//		if (type==ET_PROCESO) return false;
	...
	void Print(ostream &out, string tab="") {
		...
		if (type==ET_PROCESO) {
			...
			for(int i=0;i<n_child;i++) { 
				if (child[i]) child[i]->Print(out,tab+"   ");
			}
			out<<tab<<"FinSegun"<<endl;
		} else if (type==ET_OPCION) {
			add_tab=true;
			out<<tab<<"Opcion "<<label<<":"<<endl;
		} else if (type==ET_SI) {
...
Entity *Add(Entity *vieja, Entity *nueva, int id=-1) {
	if (id==-1) vieja->LinkNext(nueva);
	else vieja->LinkChild(id,nueva);
	return nueva;
}
...
void Load(const char *fname) {
	...
	Entity *aux=start, *aux2;
	while (getline(file,str)) {
		if (!str.size()||StartsWith(str,"PROCESO ")||str=="FINPROCESO") {
			continue;
		}
		else if (StartsWith(str,"ESCRIBIR ")) {
			aux=Add(aux,new Entity(ET_ESCRIBIR,str.substr(9)));
		}
		else if (StartsWith(str,"LEER ")) {
			aux=Add(aux,new Entity(ET_LEER,str.substr(5)));
		}
		else if (StartsWith(str,"MIENTRAS ")) {
			aux=Add(aux,new Entity(ET_MIENTRAS,str.substr(9)),0);
		}
		else if (StartsWith(str,"REPETIR ")) {
			aux=Add(aux,new Entity(ET_REPETIR,""),0);
		}
		else if (StartsWith(str,"SI ")) {
			aux=Add(aux,new Entity(ET_MIENTRAS,str.substr(3)),0);
		}
		else if (StartsWith(str,"ENTONCES ")) {
			aux=aux->parent;
		}
		else if (StartsWith(str,"SEGUN ")) {
			aux=Add(aux,new Entity(ET_SEGUN,str.substr(6)),0);
		}
		else if (StartsWith(str,"OPCION ")) {
			if (aux->type==ET_SEGUN) { // si esta despues del segun, es el primer hijo
				aux=Add(aux,new Entity(ET_OPCION,str.substr(7)),0);
			} else if (aux->type==ET_OPCION) {  // si esta despues de una opcion vacia, subir al segun
				aux=aux->parent;
				aux=Add(aux,new Entity(ET_OPCION,str.substr(7)),aux->n_child);
...
void DrawMenus() {
	...
	glBegin(GL_LINES);
		...
		if (shapebar) {
			...
			glVertex2i(win_w-2*shapebar_size_min/3,win_h/2); glVertex2i(win_w-shapebar_size_min/3,win_h/2-2*shapebar_size_min/3);
			glVertex2i(win_w-2*shapebar_size_min/3,win_h/2); glVertex2i(win_w-shapebar_size_min/3,win_h/2+2*shapebar_size_min/3);
		}
	glEnd();
	// menu
	DrawText(menu_sel==1?color_selection:color_menu,menu_size_w-240,win_h-menu_size_h+220,"Guardar Cambios");
	DrawText(menu_sel==2?color_selection:color_menu,menu_size_w-240,win_h-menu_size_h+180,"Guardar y Cerrar ");
	DrawText(menu_sel==3?color_selection:color_menu,menu_size_w-240,win_h-menu_size_h+140,"Cerrar Sin Guardar");
	DrawText(menu_sel==4?color_selection:color_menu,menu_size_w-240,win_h-menu_size_h+100,"Auto-ajustar Zoom");
