...
mxStatusBar::mxStatusBar(wxWindow *parent):wxPanel(parent,wxID_ANY,wxDefaultPosition,wxDefaultSize) {
	...
	texts[STATUS_RUNNED_OK]=st_aux(azul,"La ejecución ha finalizado sin errores.");
	texts[STATUS_RUNNED_INT]=st_aux(rojo,"La ejecución ha sido interrumpida.");
	texts[STATUS_EXAMPLE]=st_aux(azul,"Este es un ejemplo, no puede modificarlo.");
	texts[STATUS_DEBUG_RUNNING]=st_aux(azul,"El pseudocódigo está siendo ejecutado paso a paso.");
	texts[STATUS_DEBUG_PAUSED]=st_aux(azul,"La ejecución paso a paso ha sido pausada.");
	texts[STATUS_DEBUG_STOPPED]=st_aux(azul,"La ejecución paso a paso ha sido detenida.");
	texts[STATUS_DEBUG_ENDED]=st_aux(azul,"La ejecución paso a paso ha finalizado.");
	texts[STATUS_UPDATE_CHECKING]=st_aux(azul,"Buscando actualizaciones...");
	texts[STATUS_UPDATE_ERROR]=st_aux(rojo,"Error al conectarse al servidor para buscar actualizaciones.");
	texts[STATUS_UPDATE_NONEWS]=st_aux(azul,"No hay actualizaciones disponibles.");
	texts[STATUS_UPDATE_FOUND]=st_aux(azul,"Hay una nueva versión para descargar!");
	texts[STATUS_NEW_SOURCE]=st_aux(azul,"Puede utilizar el panel de comandos y estructuras para añadir instrucciones.");
	texts[STATUS_COMMAND]=st_aux(azul,"Debe completar los campos marcados con rectángulos.");
