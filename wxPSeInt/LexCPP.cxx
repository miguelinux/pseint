// Este archivo debería reemplazar al homónimo en los fuentes de wxWidgets 2.8.12 para tener el mismo coloreado
// de sintaxis, con los cambios para tomar acentos y eñes, y omitir comentarios estilo C

// Scintilla source code edit control
/** @file LexCPP.cxx
 ** Lexer for C++, and PSeInt.
 **/
// Modified Cpp lexer for PSeInt:
// 99% of the code is from:
//    Copyright 1998-2005 by Neil Hodgson <neilh@scintilla.org>
//    The License.txt file describes the conditions under which this software may be distributed.
// The lines modified for pseint have a comment saying that.
...
static void ColouriseCppDoc(unsigned int startPos, int length, int initStyle, WordList *keywordlists[],
	...
	for (; sc.More(); sc.Forward()) {
		...
		switch (sc.state) {
			...
			case SCE_C_PREPROCESSOR:
				if (sc.atLineStart && !continuationLine) {
					...
					if ((!enable_pseint && sc.Match('/', '*')) || sc.Match('/', '/')) {  // MODIFICADO PARA PSEINT
						sc.SetState(SCE_C_DEFAULT);
					}
				}
				break;
		...
		if (sc.state == SCE_C_DEFAULT) {
			...
			} else if (!enable_pseint && sc.Match('/', '*')) {   // MODIFICADO PARA PSEINT
...
static void FoldNoBoxCppDoc(unsigned int startPos, int length, int initStyle,
	...
	for (unsigned int i = startPos; i < endPos; i++) {
		...
		/*if (foldComment && (style == SCE_C_COMMENTLINE)) {  // COMENTADO PARA ZINJAI, trae problemas al comentar funciones si la llave que cierra que pegada al // y la que abre no
			if ((ch == '/') && (chNext == '/')) {
