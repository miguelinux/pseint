...
mxConfig::mxConfig(wxWindow *parent):wxDialog(parent,wxID_ANY,"Opciones del Lenguaje",wxDefaultPosition,wxDefaultSize) {
	...
	chk_force_init_vars = utils->AddCheckBox(opts_sizer,this,"No permitir utilizar variables o posiciones de arreglos sin inicializar",true);
	...
	chk_force_define_vars = utils->AddCheckBox(opts_sizer,this,"Obligar a definir los tipos de variables",true);
	...
	chk_force_semicolon = utils->AddCheckBox(opts_sizer,this,"Controlar el uso de ; al final de sentencias secuenciales",true);
	...
	chk_allow_concatenation = utils->AddCheckBox(opts_sizer,this,"Permitir concatenar variables de texto con el operador +",true);
	...
	chk_enable_string_functions = utils->AddCheckBox(opts_sizer,this,"Habilitar funciones para el manejo de cadenas",true);
	...
	chk_word_operators = utils->AddCheckBox(opts_sizer,this,"Permitir las palabras Y, O, NO y MOD para los operadores &&, |, ~ y %",true);
	...
	chk_base_zero_arrays = utils->AddCheckBox(opts_sizer,this,"Utilizar indices en arreglos y cadenas en base 0",true);
	...
	chk_allow_dinamyc_dimensions = utils->AddCheckBox(opts_sizer,this,"Permitir utilizar variables para dimensionar arreglos",true);
	...
	chk_overload_equal = utils->AddCheckBox(opts_sizer,this,"Permitir asignar con el signo igual (=)",true);
	...
	chk_enable_user_functions = utils->AddCheckBox(opts_sizer,this,"Permitir definir funciones/subprocesos",true);
	...
	chk_lazy_syntax = utils->AddCheckBox(opts_sizer,this,"Utilizar sintaxis flexible",true);
	...
	chk_coloquial_conditions = utils->AddCheckBox(opts_sizer,this,"Permitir condiciones en lenguaje coloquial",true);
	...
	chk_use_nassi_schneiderman= utils->AddCheckBox(opts_sizer,this,"Usar diagramas de Nassi-Schneiderman",false);
	...
	chk_use_alternative_io= utils->AddCheckBox(opts_sizer,this,"Usar formas alternativas para Leer y Escribir en el diagrama",false);
...
void mxConfig::OnSaveButton (wxCommandEvent & evt) {
	...
	if (dlg.ShowModal() == wxID_OK) {
		config->last_dir=wxFileName(dlg.GetPath()).GetPath();
		LangSettings l;
		l.desc=wxGetTextFromUser("Ingrese una descripción breve del perfil.","Guardar Perfil","",this);
		CopyToStruct(l);
		l.Save(dlg.GetPath());
	}
}
